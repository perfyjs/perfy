...
module.exports = (options) => {
    ...
    const globFiles = path.resolve(options.reportsFolder, options.reportsFiles);

    return new Promise((resolve, reject) => {
        glob(globFiles, (error, files) => {
            ...
            const merge = (file, index) => {
                ...
                const folderName = `${ today.getFullYear() }-${ today.getUTCMonth() }-${ today.getUTCDate() }`;
